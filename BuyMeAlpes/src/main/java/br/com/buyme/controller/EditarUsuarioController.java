...
public class EditarUsuarioController {
	...
	@FXML private Button btnFinalizar,btnTrocarSenha,btnPermissao;
    ...
    @FXML private TableColumn<Usuario, String> cAdmin;
	...
	protected void initialize(){
		...
		try{
			...
			cAdmin.setCellValueFactory(
		        new PropertyValueFactory<Usuario,String>("adminStr")
		    );
		...
		if(loginUsu != null && !loginUsu.isEmpty() && nomeUsu != null && !nomeUsu.isEmpty() && telUsu != null && !telUsu.isEmpty() &&
			...
			if(loginUsu.equals(usu.getLogin()) || verificarLogin(loginUsu)){
				if(Utils.isNumber(numUsu)){
					usuDao.editarUsuario(idUsu, loginUsu, nomeUsu, telUsu, endUsu, Integer.parseInt(numUsu), cidUsu, emailUsu, adminUsu, permissoes);
	...
	public void trocarSenha(){
		try{
			if(usu != null){
	    		...
	    		loader.setLocation(getClass().getResource("TrocarSenhaUsuarioView.fxml"));
	...
	public void editar(){
		try{
			...
			if(usu != null){
				...
				permissoes = (List<PermissoesEnum>) usu.getPermissoes();
				if(usu.isAdmin()){
	...
	public void permissoes(){
		try{
    		...
    		cadastroParametroController.setPermissoesEdicao(permissoes);
    		cadastroParametroController.preencherEdicao();
