...
public class SelecionarMotivoPerdaController {
	...
	public List<ProdMot> fechar(){
		...
		if(data != null && !data.isEmpty()){
			for(ProdMot pm: data){
				quantidadeSelecionada = quantidadeSelecionada + pm.getQuantidade();
				listaPMRetorno.add(pm);
			}
			if(quantidadeSelecionada == quantTotalPerda){
				dialogStage.close();
				return listaPMRetorno;
			}else{
				popup.getError("A quantidade selecionada é diferente da esperada!");
				return null;
			}
		}else{
			return null;
